 ...
 * NOTE: Places are now handled by db-places.js
 */
...
export async function getAllItems(options = {}) {
  ...
  let queryText = `
    SELECT
      i.*,
      CASE
        WHEN i.item_type = 'hotel' AND i.attributes->>'badge_id' IS NOT NULL THEN
    ...
    FROM items i
    WHERE i.status = $1
      AND i.item_type != 'place'  -- Places are now in dedicated places table
  `;

  const params = [status];
  let paramCount = 1;

  // Filter by item type (excluding places)
  if (item_type) {
    if (item_type === 'place') {
      throw new Error('Places are now handled by the places table. Use /api/places instead.');
    }
    paramCount++;
...
export async function createItem(itemType, data) {
  try {
    ...
    // Reject place creation - use db-places.js instead
    if (itemType === 'place') {
      throw new Error('Places must be created using db-places.js createPlace() function');
    }
...
export async function searchItems(query, itemType = null) {
  ...
  // Reject place searches - use db-places.js searchPlaces() instead
  if (itemType === 'place') {
    throw new Error('Places search must use db-places.js searchPlaces() function');
  }

  let queryText = `
    SELECT
      i.*,
      NULL as badge  -- Badge logic removed (places use dedicated table)
    FROM items i
...
export async function getItemStats() {
  const result = await sql`
    ...
    FROM items
    WHERE item_type != 'place'  -- Places are now in dedicated places table
    GROUP BY item_type
    ORDER BY item_type
  `;
