/**
 * External Links Component for Place Detail Pages
 * Displays Google Maps and TripAdvisor links (no ratings)
 *
 * Usage:
 ...
 * @param {Object} place - Place data with links
 */
export function renderRatingsSection(container, place) {
  ...
  // If no links at all, don't render the section
  if (!google_maps_url && !tripadvisor_url) {
    return;
  }

  const html = `
    <div class="ratings-section">
      <h3 class="ratings-title">Dış Bağlantılar</h3>

      <div class="ratings-cards">
        ${google_maps_url ? renderGoogleCard(google_maps_url) : ''}
        ${tripadvisor_url ? renderTripAdvisorCard(tripadvisor_url) : ''}
      </div>
    </div>
  `;
...
function renderGoogleCard(mapsUrl) {
  if (!mapsUrl) return '';

  return `
    <div class="rating-card rating-card-link-only">
      ...
      <div class="link-only-message">
        Google Maps'te mekan bilgilerini ve değerlendirmeleri görüntüle
      </div>
...
function renderTripAdvisorCard(tripAdvisorUrl) {
  if (!tripAdvisorUrl) return '';
  ...
  return `
    <div class="rating-card rating-card-link-only">
      ...
      <div class="link-only-message">
        TripAdvisor'da değerlendirmeleri ve yorumları görüntüle
      </div>
