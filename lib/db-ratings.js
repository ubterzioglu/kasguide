...
export async function getPlacesForGoogleSync() {
  const query = `
    ...
    FROM items
    WHERE google_item_id IS NOT NULL
      AND google_item_id != ''
    ORDER BY title
  `;
...
export async function saveRatingHistory(placeId, source, rating, reviewsCount) {
  const query = `
    INSERT INTO ratings_history (item_id, source, rating, reviews_count)
    VALUES ($1, $2, $3, $4)
    RETURNING id
  `;
...
export async function getRatingsSyncStats() {
  const query = `
    SELECT
      COUNT(*) FILTER (WHERE google_item_id IS NOT NULL) as total_with_google_id,
