...
(function () {
  ...
  function getId() {
    ...
    if (id) return id;
    // Preview mode: if opened from ekle.html, read draft from localStorage
    try {
      const raw = localStorage.getItem("kasguide_preview_place");
      if (raw) return "__preview__";
    } catch (e) {}
    return null;
  }
  ...
  function getPlaceById(id) {
    ...
    // Preview place (draft) comes from ekle.html via localStorage
    if (String(id) === "__preview__") {
      try {
        const raw = localStorage.getItem("kasguide_preview_place");
        if (raw) {
          ...
          return {
            ...
            title: p.title || "Mekan Adı",
            description: p.description || "",
            longText: p.longText || "",

            // Keep category compatible (string IDs)
            ...
            features: [],
            tags: [],
            trust: { verified: false, infoDate: "", disclaimer: true }
          };
        }
      } catch (e) {}
